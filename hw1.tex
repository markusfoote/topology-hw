...
\begin{align}
	...
	&\text{if } |u|=|v|=r
\end{align}
For the general case, $|u|\ne |v|$:
\begin{align}
\text{Let } u &= \alpha \hat u\\
v &=\beta \hat v\\
\text{where }& |\hat u| = |\hat v| = 1\\
\alpha \hat u + \beta \hat v & \left(\alpha + \beta\right)\left(\left(1-\frac{\beta}{\alpha + \beta}\right)\hat u + \frac{\beta}{\alpha + \beta}\hat v\right)=\gamma \left(\left(1-t\right)\hat u + t \hat v\right)\\
\text{where }& \gamma = \alpha + \beta\; , \quad t= \frac{\beta}{\alpha + \beta}\\
|u+v| = |\alpha \hat u + \beta \hat v| &= |\gamma| |\left(1-t\right)\hat u + t \hat v| \le |\gamma| 1 = |\gamma| = \alpha + \beta = |u|+|v|
\end{align}
thus the triangle inequality holds for points in a convex ball.
\end{document}
